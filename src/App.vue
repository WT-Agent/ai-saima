  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">赛马场景</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">团队规模</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">团队赛马机制与良性竞争激励指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 赛马机制与团队竞争激励专家');
...
const metricsList = [
  {
    "key": "pkMechanismFairness",
    "label": "PK规则公平竞争性"
  },
  {
    "key": "incentiveMotivationStrength",
    "label": "激励措施强吸引力"
  },
  {
    "key": "teamMoraleBoost",
  ...
  {
    "key": "performanceKPIAlignment",
    "label": "业务指标契合度"
  },
  {
    "key": "riskControlPreventive",
    "label": "竞争风险防范分"
  }
];
const guideRules = [
  {
    "title": "赛道绝对公平",
    "advice": "根据团队过往基数划分对等赛道，使用“同比增长率”而非单纯“绝对值”进行考核",
    "avoid": "切忌让新老团队在不公平基数上直接对撞"
  },
  {
    "title": "即时反馈激励",
    "advice": "建立每日战报榜单，奖励当日第一名，让正向反馈的时延降到最低",
    "avoid": "避免等到月底才公布成绩导致平时缺乏激情"
  },
  {
    "title": "坚守红线底线",
    "advice": "明确规定抢单、诋毁对手直接取消资格并追责，倡导“赛场是对手，赛后是战友”",
    "avoid": "严禁放任内耗造成团队撕裂与人才流失"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SAIMA_SCORES\][\s\S]*?\[\/SAIMA_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SAIMA_SCORES\](.*?)\[\/SAIMA_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('saima_history_records');
...
const saveHistory = () => {
  localStorage.setItem('saima_history_records', JSON.stringify(historyList.value));
};
